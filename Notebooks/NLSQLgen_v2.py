...
# Train / Test Split
accounts_train = ['비유동자산', '자산총계', '유동부채', '비유동부채', '부채총계', '이익잉여금', '자본총계', '매출액', '영업이익', '법인세차감전 순이익', '당기순이익', '자본금']
accounts_test  = ['유동자산']
            ...
            for query in querys:
                ...
                for nl in nls:
                    if ans:
                        s = [nl, query, ans]
                        # print("\t".join(s) + "\n") 
                        nlsqls_train.append(s)
                    else:
                        # print(query)
                        pass

import csv 
with open('NLSQL_train.tsv', 'wt') as f:
    writer = csv.writer(f, delimiter='\t')
    for nlsql in nlsqls_train:
            ...
            for query in querys:
                ...
                for nl in nls:
                    if ans:
                        s = [nl, query, ans]
                        # print("\t".join(s) + "\n") 
                        nlsqls_test.append(s)
                    else:
                        # print(query)
                        pass

import csv 
with open('NLSQL_test.tsv', 'wt') as f:
    writer = csv.writer(f, delimiter='\t')
    for nlsql in nlsqls_test:
        writer.writerow(nlsql) 
